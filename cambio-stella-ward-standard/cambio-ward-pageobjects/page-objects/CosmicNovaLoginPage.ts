...
const log = Factory.getLogger("Page.CosmicNovaLoginPage")
...
export class CosmicNovaLoginPage extends PageBase{
    ...
    public get(): CosmicNovaLoginPage{
        ConfigRoute.visit_page('http://cslk-ciix-81-03:8240/nova-board/#/Login');
        log.info("Step: navigate to http://cslk-ciix-81-03:8240/nova-board [:CosmicNovaLoginPage:]");
    ...
    /**
     * Step: press login button with invalid login
     * @returns {CosmicNovaSelectRolePage}
     */
    public step_pressLoginButtonBackToLogin(): CosmicNovaLoginPage{
        log.info("Step: press login button [:step_pressLoginButton:]");
        this.Helper_Waits.waitFor(2000);
        new Button(this.btnLogin).click();
        return new CosmicNovaLoginPage();
    ...
    /**
     * Validate: verify the login error message
     * @returns {CosmicNovaLoginPage}
     */
    public check_And_Validate_LoginErrorMsg(): CosmicNovaLoginPage{
        log.info("Validate: verify the login error message [:check_And_Validate_LoginErrorMsg:]");
        this.Helper_Assertion.expectToEqual(this.lblErrorMsg,'Invalid user name or password');
        return new CosmicNovaLoginPage();
    }

    /**
     * Validate: verify the login button availability
     * @returns {CosmicNovaLoginPage}
     */
    public check_And_Validate_LoginBtnDisability(): CosmicNovaLoginPage{
        log.info("Validate: verify the login button visibility [:check_And_Validate_LoginBtnDisability:]");
        this.Helper_Actions.isDisabled(this.btnLogin);
        return new CosmicNovaLoginPage();
    }
}